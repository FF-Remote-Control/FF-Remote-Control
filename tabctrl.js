...
function TabControl(tab) {
    ...
    this.worker.on('detach', this.abortPending.bind(this, "tab navigated or closed"));
}
...
TabControl.prototype.receiveTimeout = function(reqid) {
    ...
    var reply = {id: reqid, error: "timed out while waiting for reply"};
    res.callback(reply);
    delete this.pending_requests[reqid];
}
...
TabControl.prototype.abortPending = function(reason) {
    for(var reqid in this.pending_requests) {
        this.pending_requests[reqid].callback({id: reqid, error: "request aborted: " + reason});
    }
